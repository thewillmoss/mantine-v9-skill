...
// CSS Grid with responsive spans (9.x: use `gap` instead of `gutter`)
<Grid gap="md">
  <Grid.Col span={{ base: 12, sm: 6, lg: 4 }}>Responsive</Grid.Col>
</Grid>

// Separate row and column gaps (new in 9.x)
...
> **9.x note:** Grid now uses native CSS `gap` instead of negative margins.
> `overflow="hidden"` is no longer needed and can be removed.
...
{/* 9.x: use `c` style prop instead of `color` prop */}
...
| 8.x | 9.x |
|---|---|
| `<Text color="red">` | `<Text c="red">` (use `c` style prop) |
| `<Collapse in={bool}>` | `<Collapse expanded={bool}>` |
| `<Grid gutter="xl">` | `<Grid gap="xl">` (also: `rowGap`, `columnGap`) |
| `<Grid overflow="hidden">` | Remove prop (no longer needed) |
| `<Spoiler initialState={bool}>` | `<Spoiler defaultExpanded={bool}>` |
| `positionDependencies={[...]}` on Popover/Tooltip | Remove prop (automatic now) |
| `TypographyStylesProvider` | `Typography` |
| `<Notifications />` | Hovering any notification pauses all timers; use `pauseResetOnHover="notification"` for 8.x behavior |
