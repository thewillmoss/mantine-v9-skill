...
### Schema Validation (Standard Schema — Zod v4, Valibot, ArkType)

In 9.x, `@mantine/form` has built-in support for [Standard Schema](https://standardschema.dev/).
Use `schemaResolver` instead of dedicated resolver packages:
...
> **Migration note:** `zodResolver`, `yupResolver`, and `joiResolver` are no longer exported
> from `@mantine/form`. Use `schemaResolver` with any Standard Schema-compatible library.
